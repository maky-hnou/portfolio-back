...
async def get_chat(message_id: str, message_dao: MessageDAO = Depends()) -> MessageModel | None:
    message = await message_dao.get_single_row(model_class=MessageModel, chat_id=message_id)
    if message is None:
        raise HTTPException(status_code=404, detail="Chat not found")
    return message


@router.get("/message/{chat_id}", response_model=list[MessageDTO])
...
async def create_chat(message: MessageDTO, chat_dao: MessageDAO = Depends()) -> MessageModel:
